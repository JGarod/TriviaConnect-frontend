...
import { getImgUser, getMessage, getPrivateProfile, getProfile, preferencesUser, PrivateProfileResponse, ProfileResponse, receiveUpdateBasic, responseAmistad, sendChangePassword, sendUpdateBasic } from '../../interfaces/user/profile.interface';
...
//ENVIAR LOS DATOS BASICOS DEL USUARIO
  ...
  //ajustar preferencia de usuario
  ...
  //agregar usuario
  postAgregarUsuario(data: any): Observable<responseAmistad> {
    const token = this.tokenService.getToken();
    ...
    return this.http.post<responseAmistad>(`${this.baseUrl}/amistad/gestionar`, data, { headers }).pipe(
      map((response: responseAmistad) => {
        return response;
      }),
      catchError(err => {
        const error = err.error as ErrorResponse;
